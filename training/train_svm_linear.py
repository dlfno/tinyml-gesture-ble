#!/usr/bin/env python3
"""
Entrenador SVM Lineal — Clasificador de gestos IMU.

Rol en el proyecto: clasificador lineal con margen máximo.
  - Igual que Regresión Logística pero minimiza hinge loss (margen máximo)
    en lugar de log loss — teóricamente mejor generalización en fronteras ajustadas.
  - Sin dependencia de TFLite Micro
  - Latencia de inferencia < 0.1 ms
  - Establece si el criterio de margen máximo aporta frente a LR.

Pipeline:
  1. Ventanas deslizantes idénticas a CNN/RF/LR/NB (WINDOW_SIZE=100, STEP_SIZE=50)
  2. Extracción de 32 features estadísticas por ventana
  3. StandardScaler sobre los 32 features (LinearSVC requiere features escalados)
  4. LinearSVC(C=1.0, max_iter=5000, class_weight='balanced')
  5. Exporta model_svm_linear.h (C++ para Arduino via micromlgen)

Nota de exportación:
  micromlgen no soporta LinearSVC directamente. Como LinearSVC y LogisticRegression
  comparten la misma estructura de predicción — argmax(coef_ @ x + intercept_) —
  se exporta un proxy LogisticRegression con los pesos del SVM. Las predicciones
  son bit a bit idénticas.

Exporta: model_svm_linear.h (Arduino C++)
Artefactos: predictions_svm_linear.npz, class_names_svm_linear.json,
            scaler_params_svm_linear.json
"""
import os
import sys
import json
...
MODEL_DIR  = os.path.join(SCRIPT_DIR, '..', 'models', 'svm_l')
EVAL_DIR   = os.path.join(SCRIPT_DIR, '..', 'eval', 'svm_l')
...
# ── 6. Entrenamiento ──────────────────────────────────────────────────────────
print(f'\nFASE 5b — Entrenamiento LinearSVC (C={C_REG}, max_iter={MAX_ITER})')
...
json.dump(class_names,
          open(os.path.join(MODEL_DIR, 'class_names_svm_linear.json'), 'w'), indent=2)
print(f'  Guardado: class_names_svm_linear.json')

np.savez(os.path.join(MODEL_DIR, 'predictions_svm_linear.npz'),
         y_test=y_test, y_pred=y_pred_test,
         class_names=np.array(class_names))
print(f'  Guardado: predictions_svm_linear.npz')

json.dump({'feature_names': FEATURE_NAMES,
           'mean': scaler.mean_.tolist(),
           'std':  scaler.scale_.tolist()},
          open(os.path.join(MODEL_DIR, 'scaler_params_svm_linear.json'), 'w'), indent=2)
print(f'  Guardado: scaler_params_svm_linear.json')

# ── 10. Exportar modelo C++ para Arduino ──────────────────────────────────────
print('\nFASE 9 — Exportar modelo C++ para Arduino (micromlgen via proxy LR)')
print('  LinearSVC y LogisticRegression comparten estructura argmax(coef_@x + b).')
print('  El proxy exporta los pesos del SVM con el generador de LR.')

model_h_path = os.path.join(MODEL_DIR, 'model_svm_linear.h')
try:
    from micromlgen import port

    # Proxy: copiar coef_/intercept_ del SVM a un objeto LogisticRegression vacío.
    # Las predicciones son idénticas porque ambos calculan argmax(coef_ @ x + intercept_).
    lr_proxy = LogisticRegression()
    lr_proxy.coef_      = clf.coef_
    lr_proxy.intercept_ = clf.intercept_
    lr_proxy.classes_   = clf.classes_
...
SECONDARY = '#6b7280'
TEXT      = '#e5e7eb'
PRIMARY   = '#06b6d4'   # cyan para SVM Lineal
...
ax.set_title(f'SVM Lineal — Test Accuracy: {acc_test:.1%}',
             fontweight='bold', color=TEXT, pad=10)
plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
plt.tight_layout()
cm_path = os.path.join(EVAL_DIR, 'confusion_matrix_svm_linear.png')
...
fig.suptitle('SVM Lineal — Per-Class Metrics', fontsize=14,
             fontweight='bold', color=TEXT)
plt.tight_layout()
metrics_path = os.path.join(EVAL_DIR, 'per_class_metrics_svm_linear.png')
fig.savefig(metrics_path, dpi=150, bbox_inches='tight', facecolor=BG_DARK)
plt.close()
print(f'  Guardado: {os.path.relpath(metrics_path)}')

# Heatmap de coeficientes (clases × features) — igual que LR
...
ax.set_yticklabels(class_names, fontsize=9)
ax.set_title('SVM Lineal — Coeficientes por Clase (rojo=negativo, cyan=positivo)',
             fontweight='bold', color=TEXT, pad=10)
plt.colorbar(im, ax=ax, fraction=0.015, pad=0.02)
plt.tight_layout()
coef_path = os.path.join(EVAL_DIR, 'coefficients_svm_linear.png')
...
md = f"""# Reporte de Evaluación — SVM Lineal

## Resumen
- **Algoritmo:** LinearSVC (hinge loss, OvR multiclase)
- **Hiperparámetros:** C={C_REG}, max_iter={MAX_ITER}, class_weight='balanced'
- **Iteraciones:** {clf.n_iter_}
...
- **Rol:** Clasificador lineal con margen máximo
- **Dependencia de TFLite:** ninguna (C++ puro via micromlgen)
- **Exportación:** proxy LogisticRegression con pesos del SVM (predicciones idénticas)
- **Formato:** `model_svm_linear.h` (Arduino C++)
- **Nota de firmware:** aplicar Z-score con `scaler_params_svm_linear.json` antes de `SVMLinearModel::predict()`
...
| Archivo | Tamaño |
|---------|--------|
| `model_svm_linear.h` | {model_h_kb:.2f} KB (texto, no compilado) |
| `scaler_params_svm_linear.json` | < 5 KB |
...
print()
print('=' * 60)
print('Entrenamiento SVM Lineal completado.')
print('=' * 60)
print(f'  Test accuracy : {acc_test*100:.2f}%')
print(f'  CV accuracy   : {cv_scores.mean()*100:.2f}% ± {cv_scores.std()*100:.2f}%')
print(f'  model_svm_linear.h : {model_h_kb:.2f} KB')
print()
print('Próximo paso — copiar model_svm_linear.h al sketch Arduino:')
print(f'  cp models/svm_l/model_svm_linear.h arduino/tinyml_ble_svm_l/model_svm_linear.h')
